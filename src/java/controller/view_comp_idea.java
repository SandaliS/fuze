 ...
 * @author Saroj
 */
@WebServlet(name = "view_comp_idea", urlPatterns = {"/view_comp_idea"})
public class view_comp_idea extends HttpServlet {
  ...
  @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            RequestDispatcher r = request.getRequestDispatcher("comp_view_idea.jsp");
    ...
    public List<unieventbean> progectdetails(String name) throws Exception {
        ...
        ResultSet rs = DB.search("SELECT * FROM `comp_event`,`company` WHERE `company`.`cid` = comp_event.uid AND `comp_event`.`uid` = '"+id+"'");
        while (rs.next()) {
